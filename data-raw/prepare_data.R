...
# ------------------------------------------------------------
# 5. speeches: committee speech sample (ALL standing committees)
# Source: data-raw/speeches_all_committees.csv
#   (generated by data-raw/process_all_speeches.py from
#    who-owns-nk-policy xlsx archives, 16th-20th assemblies)
# Stratified sample of 1,500 per assembly (10,500 total)
# ------------------------------------------------------------
raw_speeches <- read_csv(
  "data-raw/speeches_all_committees.csv",
  show_col_types = FALSE
)

set.seed(42)
speeches <- raw_speeches |>
  filter(nchar(speech) >= 50, !is.na(date)) |>
  group_by(assembly) |>
  slice_sample(n = 1500) |>
  ungroup() |>
  transmute(
  ...
  # Step 1: match by name + assembly
  left_join(xwalk, by = c("speaker_name" = "name", "assembly")) |>
  # Step 2: fallback to name-only for unmatched rows (helps 16th-17th)
